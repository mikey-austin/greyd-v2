...
// committed file; if it fails after a change to package kv or to this
// driver, the change broke compatibility with existing databases.
//
// Regenerate (only when a format change is intended) with
//
//	GREYD_UPDATE_GOLDEN=1 go test ./adapters/db/bolt -run TestUpdateGolden
const (
...
// goldenEntry pairs a key with the data the fixture holds for it.
type goldenEntry struct {
	k core.Key
	d core.Data
}

// goldenEntries is the fixture's content. Spamtraps and domains are stored
// with zero data, as greydb writes them.
func goldenEntries() []goldenEntry {
	...
	return []goldenEntry{
		...
		{k: core.MailKey("trap@example.net")},
		{k: core.DomainKey("example.net")},
	}
}
...
func TestUpdateGolden(t *testing.T) {
	...
	// bbolt grows the file to its minimum mmap window (32 KiB) regardless
	// of how many pages are in use; drop the never-written tail so the
	// committed fixture only holds the pages the meta page references.
	if err := trimBolt(path); err != nil {
		t.Fatal(err)
	}
	fi, err := os.Stat(path)
	if err != nil {
...
// trimBolt truncates a closed bbolt file to its high-water mark (the page
// count of the current meta page times the page size).
func trimBolt(path string) error {
	...
	return os.Truncate(path, used)
}

// copyFixture copies the committed file into a temporary directory so the
// test never opens (and bbolt never locks or grows) the file in the tree.
...
func openGolden(t *testing.T, dir string, mode core.OpenMode) core.Store {
	t.Helper()
	cfg := newConfig(dir)
	cfg.SetStr("db_name", "database", goldenName)
	s, err := core.OpenStore(cfg, core.StoreOptions{})
...
func TestGoldenFixture(t *testing.T) {
	...
	// The domain suffix lookup works against the stored domain.
