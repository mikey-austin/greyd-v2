...
func testConfig(t *testing.T) *config.Config {
	...
	cfg.SetStr("name", "database", c.DBName)
	cfg.SetStr("user", "database", c.User)
	cfg.SetStr("pass", "database", c.Passwd)
	if c.Net == "unix" {
		cfg.SetStr("socket", "database", c.Addr)
	} else {
		host, port, err := net.SplitHostPort(c.Addr)
		if err != nil {
			t.Fatalf("%s address %q: %v", dsnEnv, c.Addr, err)
		}
		p, err := strconv.Atoi(port)
		if err != nil {
			t.Fatalf("%s port %q: %v", dsnEnv, port, err)
		}
		cfg.SetStr("host", "database", host)
		cfg.SetInt("port", "database", p)
	}
	return cfg
}
...
func TestConfigDefaults(t *testing.T) {
	cfg := config.New()
	c := New(cfg, core.StoreOptions{}).Config()
	if c.Net != "tcp" || c.Addr != "localhost:3306" || c.DBName != "greyd" {
		t.Fatalf("defaults = %s %s %s", c.Net, c.Addr, c.DBName)
	}
	cfg.SetStr("socket", "database", "/run/mysqld/mysqld.sock")
	cfg.SetStr("user", "database", "u")
	cfg.SetStr("pass", "database", "p")
	c = New(cfg, core.StoreOptions{}).Config()
	if c.Net != "unix" || c.Addr != "/run/mysqld/mysqld.sock" || c.User != "u" || c.Passwd != "p" {
		t.Fatalf("socket config = %+v", c)
	}
}
