#!/bin/sh
#
# greyd OpenBSD smoke test: pf firewall driver, sqlite database, pledge.
#
...
#      driver, sqlite under /var/greyd, chroot, drop_privs and sandbox = 1
#      so that every process pledges itself,
#   3. loads a minimal pf.conf defining the greyd-whitelist and
#      greyd-greytrap tables with an rdr-to of loopback port 2525 to
#      greyd's 8025 (exercises DIOCNATLOOK under pledge "pf"),
#   4. whitelists an address with greydb, starts greyd -F and checks:
#        - the SMTP dialogue through the redirected port ends in 451,
#        - greyd (all three processes) is still alive afterwards: a pledge
#          violation kills the offending process with SIGABRT,
...
go version
pass "OpenBSD $(uname -r) with $(go version | cut -d' ' -f3), pf device present"
...
pass "programs present with pf and sqlite drivers"
...
cat >"$CONF" <<EOF
#
# greyd OpenBSD smoke test configuration (generated by run-openbsd.sh).
#
debug = 1
verbose = 1
daemonize = 0
...
cat >"$PFCONF" <<EOF
# greyd smoke test ruleset (generated by run-openbsd.sh).
...
# process recovers the original destination with DIOCNATLOOK.
pass in quick on lo0 proto tcp to port $RDR_PORT rdr-to 127.0.0.1 port $SMTP_PORT
pass
EOF
...
step "start greyd -F (sandbox = 1: every process pledges)"
...
step "greyd survived the dialogue (no pledge violation)"
sleep 1
is_alive "$GREYD_PID" || die "greyd main process died"
n=$(ps -axo command | grep -c "^$BIN/greyd ")
[ "$n" -ge 3 ] || die "expected 3 greyd processes, found $n (a pledge violation aborts a process)"
if dmesg | tail -n 50 | grep -q 'greyd.*pledge'; then
    die "dmesg reports a pledge violation"
fi
if log_has "child process exited"; then
    die "greyd logged a child exit"
fi
pass "all three processes alive, no pledge violation reported"
